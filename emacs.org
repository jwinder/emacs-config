...
(transient-define-prefix jw-runbook-menu ()
  ["Runbook"
   ...
   ("K" "cleanup         kill dev buffers & processes" jw-runbook-run-cleanup)
   ])
...
(cl-defun jw-runbook-setup-type (type &key install dev start run compile test-compile test test-only lint command last-command repl shell-command eshell cleanup)
  ...
  (when cleanup (fset (intern (format "jw-runbook-run-cleanup:%s" type)) cleanup)))
...
(jw-runbook-setup-type "js"
                       ...
                       :cleanup (lambda (arg) (jw--run-cmd "binci --cleanup" :tail-output t)))
#+END_SRC
