...
(use-package transient :pin melpa-stable)
...
(setq jw--font-size nil)
(defun jw--set-font-size (size)
  (when (and (s-numeric? size) (>= (string-to-number size) 8) (<= (string-to-number size) 72))
    (setq jw--font-size size)
    (set-face-attribute 'default nil :font (jw--font-name size))))
(defun jw--get-font-size ()
  jw--font-size)

(setq jw--transparency nil)
(defun jw--set-transparency (value)
  (when (and (s-numeric? value) (>= (string-to-number value) 0) (<= (string-to-number value) 100))
    (setq jw--transparency value)
    (set-frame-parameter (selected-frame) 'alpha (string-to-number value))))
(defun jw--get-transparency ()
  jw--transparency)
...
(cl-defun jw--run-cmd (command &key process-name before-process-creation after-process-creation after-process-finish delete-tmp-script tail-output)
...
(add-hook 'linum-before-numbering-hook (lambda () (set-face-attribute 'linum nil :foreground "#5683a2" :background "black")))
...
(defun font-size-set ()
  ...
  (let* ((current (jw--get-font-size))
         (updated (read-string (format "8 to 72 (default 14, big 18, current %s): " current) current)))
    (jw--set-font-size updated)))

(defun font-size-normal () (interactive) (jw--set-font-size "14"))
(defun font-size-big () (interactive) (jw--set-font-size "18"))
(defun font-size-increase () (interactive) (jw--set-font-size (number-to-string (1+ (string-to-number (jw--get-font-size))))))
(defun font-size-decrease () (interactive) (jw--set-font-size (number-to-string (1- (string-to-number (jw--get-font-size))))))

(defun transparency-set ()
  (interactive)
   (let* ((current (jw--get-transparency))
          (updated (read-string (format "0 full to 100 no transparency (default 75, current %s): " current) current)))
     (jw--set-transparency updated)))

(defun transparency-normal () (interactive) (jw--set-transparency "75"))
(defun transparency-off () (interactive) (jw--set-transparency "100"))
(defun transparency-increase () (interactive) (jw--set-transparency (number-to-string (1- (string-to-number (jw--get-transparency))))))
(defun transparency-decrease () (interactive) (jw--set-transparency (number-to-string (1+ (string-to-number (jw--get-transparency))))))
...
(define-key ctl-x-4-map (kbd "<left>") 'transparency-increase)
(define-key ctl-x-4-map (kbd "<right>") 'transparency-decrease)

(global-set-key (kbd "<C-wheel-up>") 'font-size-increase)
(global-set-key (kbd "<C-wheel-down>") 'font-size-decrease)
(global-set-key (kbd "<C-wheel-left>") 'transparency-increase)
(global-set-key (kbd "<C-wheel-right>") 'transparency-decrease)
...
;; todojoe -- this acts sorta weird, it doesn't exit the popup when starting the process
;; and for some reason, the universal arg was set to C-p, breaking C-p navigation -- fixed on master
;; (setq transient-enable-popup-navigation t)
;; (define-key transient-map (kbd "C-p") nil)
;; (define-key transient-map (kbd "C-u") 'universal-argument)
