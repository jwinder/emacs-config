...
- todo -- the sbt command should prompt for a path to an sbt project if currently not in an sbt project
- todo -- also, maybe add some rich functionality to invoke `sbt-new` if wanted
