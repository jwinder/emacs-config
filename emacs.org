...
(defun counsel-find-file-dwim (arg)
  ...
  (if (region-active-p)
    ...
    (if arg
        (counsel-find-file (thing-at-point 'filename))
      (if (thing-at-point 'url)
          (browse-url (thing-at-point 'url))
        (counsel-find-file)))))

(defun swiper-dwim (arg)
  (interactive "P")
  (if (region-active-p)
...
(defun swiper-backward-dwim (arg)
  (interactive "P")
  (if (region-active-p)
      (swiper-backward (buffer-substring-no-properties (region-beginning) (region-end)))
    (if arg
        (swiper-thing-at-point)
      (swiper-backward))))
