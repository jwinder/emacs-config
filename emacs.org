...
(when (executable-find "pt")
  (setq helm-grep-ag-command "pt -i -e --nocolor --nogroup %s %s %s")
  (setq helm-ag-base-command "pt -i -e --nocolor --nogroup"))

;; todo; remove helm-ag from deps if helm-projectile switches helm-do-ag to helm-do-grep-ag (https://github.com/bbatsov/helm-projectile/issues/61)
