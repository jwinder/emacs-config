...
(defun jw--buffers-cycle-to-next (mode cycle-backward &optional create-new args-list)
...
Negative prefix arg C--: Similar behavior to no prefix argument, except the cycling behavior is in reverse."
  (pcase (prefix-numeric-value arg)
    ...
    (-1 (jw--buffers-cycle-to-next mode t create-new nil))
    (_ (jw--buffers-cycle-to-next mode nil create-new nil))))
