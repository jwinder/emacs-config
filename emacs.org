...
(setq echo-area-ignored-patterns
      '("Auto saving"
        "No changes need to be saved"
        "You can run the command"
        "Shell command succeeded with no output"))

(defun message--advice--ignore-patterns (message-function &rest args)
  "Do not echo any messages matching a pattern in `echo-area-ignored-patterns'."
  (when args ;; car args = format-string, cdr args = messages
    (let ((current-formatted-message (apply 'format args)))
      (unless (member-if '(lambda (pattern) (search pattern current-formatted-message)) echo-area-ignored-patterns)
        (apply message-function args)))))

(advice-add 'message :around #'message--advice--ignore-patterns)
#+END_SRC
