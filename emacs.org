...
;; todo -- expand this and turn it into a full theme, using afternoon-theme as a base.
;; afternoon theme is getting outdated & doesn't seem to be maintained much.

;; overrides
(let* ((font-name (jw--font-name jw--default-font-size))
       (background "black")
       (foreground "#eaeaea") ;; from afternoon-theme
       (cursor "dark gray")
       )
  ...
  (custom-set-faces
   ...
   `(mode-line ((t (:font ,font-name :foreground "#7db5d6" :background "#22083397778B" :box (:style released-button)))))
   `(mode-line-inactive ((t (:foreground "gray" :background "#263238" :box (:style released-button)))))
   `(mode-line-buffer-id ((t (:foreground ,foreground))))
   `(mode-line-highlight ((t (:foreground "#7db5d6"))))
   `(header-line ((t (:foreground ,foreground :background "#005858"))))
...
;; todojoe - turn off for now, toggling warnings & errors
;; not sure which binding i want for this. maybe put it in runbook menu.
