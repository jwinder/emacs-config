...
(defun jw--mode-line-make-buffer-id ()
  (let* ((b (current-buffer))
         ...
         (display-name (if jw-mode-line-config-show-pwd (format "%s · %s" pwd buffer-name) buffer-name)))
...
(define-transient-command cmd-menu ()
  ["Run command"
...
(define-transient-command gist-menu ()
  ["Gist"
...
;; jank cache to help with magit/transient. there is an edge case where exiting a transient by cancelling might leave this variable set. but the keymap command & transient clear it.
...
(defun jw-runbook-command ()
  (interactive)
  (setq jw-runbook--target-location nil)
  (set-transient-map jw-runbook-keymap))

(global-set-key (kbd "C-c n") 'jw-runbook-command)
...
(define-key jw-runbook-keymap (kbd "n") 'jw-runbook-run-command)
...
(define-transient-command jw-runbook-menu ()
  ["Runbook"
   ...
   ("n" "command         provide an arbitrary command" jw-runbook-run-command)
...
(global-set-key (kbd "C-c N") 'jw-runbook-menu-open)
...
(ivy-add-actions
 'counsel-projectile-switch-project
 '(("C-cn" counsel-projectile-switch-project--jw-runbook-menu-action "jw-runbook")))

(cl-defun jw-runbook-setup-type (type &key install dev start run compile test test-only command last-command repl)
  (interactive)
  ...
  (fset (intern (format "jw-runbook-run-shell-command:%s" type)) (lambda (arg) (cmd-dwim arg)))
  (fset (intern (format "jw-runbook-run-eshell:%s" type)) (lambda (arg) (eshell-dwim nil))))
...
#+BEGIN_SRC emacs-lisp
;; todojoe - npm/yarn/binci things, lsp stuff

;; idea -- prefix arg forces yarn commands, regardless of binci.yml being present

(jw-runbook-setup-type "js"
                       :install 'ignore
                       :dev 'ignore
                       :start 'ignore
                       :run 'ignore
