...
(use-package helm :pin melpa) ;; 3.0 had a bug with helm-ff-delete-files. can unpin when 3.1 is released.
(use-package helm-ag)
...
(defun org-babel-src-yasnippet (yas-shortcut &optional ob-lang-override)
  (let* ((yas-shortcut-string (if (symbolp yas-shortcut) (symbol-name yas-shortcut) (yas-shortcut)))
         (ob-lang-string (if ob-lang-override
                             (if (symbolp ob-lang-override) (symbol-name ob-lang-override) ob-lang-override)
                           yas-shortcut-string))
         (yas-file-location (f-expand (format "org-mode/%s" yas-shortcut-string) yas-dynamic-snippets-dir))
         (yas-file-contents-format-string "# -*- mode: snippet -*-\n# name: %s\n# key: %s_\n# --\n#+BEGIN_SRC %s\n$0\n#+END_SRC")
         (yas-file-contents (format yas-file-contents-format-string yas-shortcut-string yas-shortcut-string ob-lang-string)))
    (jw--write-to-file yas-file-contents yas-file-location)))
...
(org-babel-src-yasnippet 'translate-spanish-to-english "translate :src es :dest en")
(org-babel-src-yasnippet 'translate-english-to-spanish "translate :src en :dest es")
