...
;; org-startup-folded no longer folds src blocks, this hides blocks everytime a heading is cycled.
(defun org-cycle--advice--hide-subtree-blocks (&rest args)
  (when (org-at-heading-p)
    (org-block-map (apply-partially 'org-fold-hide-block-toggle 'hide)
                   (org-entry-beginning-position)
                   (org-entry-end-position))))
(advice-add 'org-cycle :after 'org-cycle--advice--hide-subtree-blocks)
  ...
  (if (f-exists? "pnpm-lock.yaml") "pnpm"
    (if (f-exists? "yarn.lock") "yarn"
      (if (f-exists? "package-lock.json") "npm"
        (error "Could not figure out package manager script runner")))))
