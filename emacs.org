...
opinionated compile/test/etc. commands that depend on the project type, inspired by org-babel-execute implementations
#+BEGIN_SRC emacs-lisp
...
(defun jw-js-run-raw-command (arg &optional command)
  ...
  (let ((cmd (or command (read-shell-command "Run: "))))
    ...
    ;; todojoe - compile/compilation-mode instead of just dispatching a shell command
