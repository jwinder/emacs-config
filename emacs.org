...
(defun jw-make--get-targets-list ()
  "Reads Makefile in the current directory and returns a list of primary targets.
It looks for lines starting with alphanumeric characters followed by a colon,
and filters out special targets starting with a dot (like .PHONY)."
  (when (file-exists-p "Makefile")
    (with-temp-buffer
      (insert-file-contents "Makefile")
      (goto-char (point-min))
      (let (targets)
        ;; not perfect, tries to get each make target, skips = assignments
        (while (re-search-forward "^\\([a-zA-Z0-9_-]+\\):[ \t]*\\([^=]\\|$\\)" nil t)
          (let ((target (match-string 1)))
            (unless (string-prefix-p "." target)
              (cl-pushnew target targets :test 'equal))))
        (nreverse targets) ;; return targets in the order they appeared in the Makefile
        ))))

(defun jw-make-insert-makefile-targets-as-org-links ()
  (interactive)
  (let* ((targets (jw-make--get-targets-list))
    ...
    (if (null targets)
      ...
      (progn
        (insert (format "- [[elisp:(compile \"%s\")][%s]]\n" runner runner)) ;; default make command
...
#+BEGIN_SRC emacs-lisp
(defun jw-char-insert--set-insert-functions (name-char-pairs)
  (declare (indent defun))
  (dolist (pair name-char-pairs)
    (let* ((name (car pair))
           (char (cdr pair))
           (func-name (intern (format "jw-char-insert--%s" (downcase (s-replace " " "-" name))))))
      (fset func-name
            `(lambda (arg)
               (interactive "P")
               (if arg (kill-new ,char) (insert ,char))
               (message "%s %s %s" (if arg "Killed" "Inserted") ,name ,char))))))

;; add more as needed
(jw-char-insert--set-insert-functions
  '(("small bullet" . "·")
    ("big bullet" . "•")
    ("ellipsis" . "…"))
  )
...
(defun projectile-add-jw-project-org-link-creators ()
  (interactive)
  (when (file-exists-p "package.json")
    (insert "\n\n" "[[elisp:(progn (open-line-next) (newline) (jw-js-insert-package-json-scripts-as-org-links))][jw-js-insert-package-json-scripts-as-org-links]]"))
  (when (file-exists-p "Makefile")
    (insert "\n\n" "[[elisp:(progn (open-line-next) (newline) (jw-make-insert-makefile-targets-as-org-links))][jw-make-insert-makefile-targets-as-org-links]]"))
  )

(defun projectile-open-jw-project-org-file ()
  (interactive)
  (let* ((filename ".jw-project.org")
    ...
    (when (not file-exists)
      ...
      (projectile-add-jw-project-org-link-creators)
      (insert "\n"))
    (save-buffer)))
...
(defun counsel-projectile-switch-project--jw-project-org (project)
  (projectile-with-default-dir project
    (call-interactively 'projectile-open-jw-project-org-file)))
...
(defun jw-js-insert-package-json-scripts-as-org-links ()
  (interactive)
