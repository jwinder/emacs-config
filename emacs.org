...
(defun jw--run-cmd (full-command &optional buffer-name just-toggle-back-if-current-buffer-matches before-buffer-process-creation after-buffer-process-creation)
  "A wrapper around `async-shell-command'.
If `buffer-name' already exists, this will just switch to it.
If `just-toggle-back-if-current-buffer-matches' is `non-nil' then we just switch back to `(other-buffer)'."
  (if (and just-toggle-back-if-current-buffer-matches buffer-name (string= (buffer-name) buffer-name))
    ...
    (if (and buffer-name (get-buffer buffer-name))
      ...
      (let* ((prepared-cmd (string-trim full-command))
             (name-for-buffer (or buffer-name (concat "*" prepared-cmd "*")))
        ...
        (if (string= "" prepared-cmd)
          ...
          (progn
            ...
            (async-shell-command prepared-cmd buffer)
...
(defun cmd (command)
  ...
  (jw--run-cmd command (format "*%s*" (replace-regexp-in-string "\n" " && " command))))

(defun cmd-tmux (command)
  (interactive "sCommand: ")
  (call-process "tmux" nil nil nil "new-session" "-d" "-s" "emacs")
  (call-process "tmux" nil nil nil "send-keys" "-t" "emacs" command "C-m")
  (message "Sent to tmux session: emacs"))
...
(defun org-babel-execute:cmd (body params)
  (let* ((bg-option (assoc :bg params))
         (bg (and bg-option (not (string= (cdr bg-option) "no"))))
         (tmux-option (assoc :tmux params))
         (tmux (and tmux-option (not (string= (cdr tmux-option) "no")))))
    (if tmux
        (progn (cmd-tmux body) "Sent to tmux session: emacs")
      (progn
        (cmd-dwim nil body)
        (when bg (switch-to-buffer (other-buffer)))
        "Running command"))))
...
(defun sonic-pi-server ()
  ...
  (jw--run-cmd "sonic-pi run-server" sonic-pi-server-buffer-name t nil
               '(lambda ()
