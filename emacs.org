  ...
  (let* ((raw-query
          (if (region-active-p)
            ...
            (pcase (prefix-numeric-value arg)
              (16 ":help")
              (4 ":list")
              (_ (read-string "Command (git/ln/:help/:list/~search/etc or /python & /python/:learn & python/quicksort - curl cheat.sh/")))))
         (query (s-replace "\s" "+" (s-trim raw-query))))
...
(defun transparency-set (value)
  (interactive "n0 - 100 (default 75): ")
  (jw--set-transparency value))
