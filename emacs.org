...
(defun jw--run-cmd (command &optional
                            buffer-name
                            just-toggle-back-if-current-buffer-matches
                            before-buffer-process-creation
                            after-buffer-process-creation)
  "A wrapper around `make-comint-in-buffer'.
If `buffer-name' is explicitly provided and already exists, this will just switch to it (useful for long running server processes, for example).
      ...
      (let* ((prepared-cmd (string-trim command))
        ...
        (if (string= "" prepared-cmd)
          ...
          (progn
            ...
            (apply 'make-comint-in-buffer prepared-cmd buffer command-name nil command-args)
...
M-x `cmd-dwim' will run an async shell command in a new buffer using comint.
...
Otherwise, the shell command must be read from prompt."
  (interactive "P")
  (let ((prepared-cmd (or command (if (region-active-p)
    ...
    (if arg
      ...
      (jw--run-cmd prepared-cmd nil nil)
      (cmd prepared-cmd))))
