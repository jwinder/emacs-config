...
;; using only for eglot typescript right now right now. maybe setup with certain checkers/linters in the future
...
(defun eshell-kill-all ()
  (interactive)
  (jw--shells-kill-buffers 'eshell-mode))
...
(when (package-installed-p 'vterm)
  ...
  (defun vterm-kill-all ()
    (interactive)
    (jw--shells-kill-buffers 'vterm-mode))
...
Negative prefix arg C--: Similar behavior to no prefix argument, except the cycling behavior is in reverse."
  (pcase (prefix-numeric-value arg)
    (4 (funcall-interactively create-new arg))
    (-1 (jw--shells-cycle-to-next mode t create-new))
    (_ (jw--shells-cycle-to-next mode nil create-new))))

(defun jw--shells-kill-buffers (mode)
  (-each (jw--shells-buffer-list mode) (lambda (b) (kill-buffer b))))

(defun jw--shells-extract-buffer-name-digit (buffer)
  (string-to-number (or (car (s-match "[[:digit:]]+" (buffer-name buffer))) "-1")))

(defun jw--shells-buffers-list-ordering (b1 b2)
  (< (jw--shells-extract-buffer-name-digit b1) (jw--shells-extract-buffer-name-digit b2)))

(defun jw--shells-buffer-list (mode)
  (-sort 'jw--shells-buffers-list-ordering (-filter (lambda (buffer) (eq (buffer-local-value 'major-mode buffer) mode)) (buffer-list))))

(defun jw--shells-buffer-names-list (mode)
  (-map (lambda (b) (buffer-name b)) (jw--shells-buffer-list mode)))

(defun jw--shells-cycle-to-next (mode cycle-backward create-new)
  (let ((shells (jw--shells-buffer-names-list mode)))
    (if (or (not (eq major-mode mode)) (not shells))
        (funcall-interactively create-new nil) ;; switch to or create the first shell/term if none exist
      (let* ((num-shells (length shells))
             (idx (or (-elem-index (buffer-name) shells) num-shells))
             (next-idx (mod (if cycle-backward (- idx 1) (+ idx 1)) num-shells))
             (next-shell (nth next-idx shells)))
...
(let* ((head (car counsel-projectile-switch-project-action))
       ...
       (fixed-actions (counsel-projectile--filter-out-action-keys original-actions (list "E" "Oc" "Oa" "si" "ss" "sr" "sg"))))
...
(define-key projectile-mode-map (kbd "C-c p s s") 'counsel-projectile-rg)
