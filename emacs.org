...
(cl-defun jw--run-cmd (command &key process-name buffer-name before-process-creation after-process-creation after-process-finish tail-output kill-process-buffer delete-tmp-script)
        ...
        (let* ((tmp-script-rel-filename (replace-regexp-in-string "[^a-zA-Z0-9]+" "-" cmd-process-name))
          ...
          (letrec ((process-buffer (get-buffer-create cmd-buffer-name))
                   ...
                   (run-cmd (lambda ()
                              ...
                              (insert cmd-buffer-name "\n\n")
                              (message cmd-buffer-name)
                              ...
                              (let ((proc (get-buffer-process process-buffer)))
                                ...
                                (jw--do-when-process-finishes (get-buffer-process process-buffer)
                                                              `(lambda (proc)
                                                                 (message "%s finished" ,cmd-buffer-name)
