...
(defun jw-js-find-project-root ()
  (let ((root-dir (ignore-errors (vc-find-root (jw--pwd) "package.json"))))
    (if root-dir (file-truename root-dir) nil)))
...
(defun jw-js-execute (&optional cmd-start)
  ...
  (let ((default-directory (or (jw-js-find-project-root) (jw--vc-root-dir) (jw--pwd))))
    (jw--run-cmd (read-shell-command "Command: " (if cmd-start (concat cmd-start " ") "")))))
...
(global-set-key (kbd "C-c e") 'jw-js-command-prefix) ;; e = ecmascript
(define-key jw-js-keymap (kbd "e") 'jw-js-execute)
(define-key jw-js-keymap (kbd "n") 'jw-js-node)
(define-key jw-js-keymap (kbd "m") 'jw-js-npm)
(define-key jw-js-keymap (kbd "y") 'jw-js-yarn)
(define-key jw-js-keymap (kbd "b") 'jw-js-binci)
#+END_SRC
...
(ivy-add-actions
 ...
 '(("!" counsel-projectile-switch-project--cmd-dwim-action "cmd-dwim in project root")
   ...
   ("C-cey" counsel-projectile-switch-project--jw-js-yarn-action "js yarn")
   ("C-ceb" counsel-projectile-switch-project--jw-js-binci-action "js binci")
   ))
