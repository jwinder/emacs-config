...
function to help iterate through a table of keybindings and set them
#+BEGIN_SRC emacs-lisp
(defun keybindings-org-table-set-row (key &optional fn scope)
  (let ((keymap (cond ((s-equals? scope "local") (current-local-map))
                      ((or (s-blank-str? scope) (s-equals? scope "global")) (current-global-map))
                      ((boundp (intern-soft scope)) (eval (intern-soft scope)))))
        (def (if (or (s-blank-str? fn) (s-equals? fn "nil")) nil (intern-soft fn))))
    (if keymap
        (define-key keymap (kbd key) def)
      (message "Cannot deduce keymap when setting function %s to key %s in map %s" fn key scope))))

(unless (boundp 'jw-all-cached-keys) (setq jw-all-cached-keys nil))
(defun keybindings-org-table-cache-binding (rows &optional binding)
  (-each rows (lambda (r) (add-to-list 'jw-all-cached-keys r)))
  (when binding (set binding rows)))

(defun keybindings-org-table-set (rows &optional binding)
...
C-c b gets set to fn-2 in the current local map (useful if invoked inside of a hook).
...
`binding' is optional. When provided, it will be set globally with the value of the keybindings list.

`jw-all-cached-keys' contains every keybinding coming through this function (whether called in a hook or not).
      "
  (keybindings-org-table-cache-binding rows binding)
  (-each rows
    (lambda (row)
      (keybindings-org-table-set-row (car row) (s-presence (cadr row)) (s-presence (nth 2 row))))))
#+END_SRC
...
(keybindings-org-table-set base-keys 'jw-base-keys)
#+END_SRC
...
(keybindings-org-table-set eshell-keys 'jw-eshell-keys)
...
(keybindings-org-table-set yas-keys 'jw-yas-keys)
...
helm-eshell-hook-keys
#+NAME: helm-eshell-hook-keys
| Key   | Function            | Scope           |
|-------+---------------------+-----------------|
| <tab> | helm-esh-pcomplete  | eshell-mode-map |
| M-p   | helm-eshell-history | eshell-mode-map |

#+HEADER: :var helm-keys=helm-keys
#+HEADER: :var helm-eshell-hook-keys=helm-eshell-hook-keys
#+BEGIN_SRC emacs-lisp
(require 'helm)
(require 'helm-config)
...
(keybindings-org-table-set helm-keys 'jw-helm-keys)

(keybindings-org-table-cache-binding helm-eshell-hook-keys 'jw-helm-eshell-hook-keys)
(add-hook 'eshell-mode-hook (lambda ()
                              (keybindings-org-table-set jw-helm-eshell-hook-keys)))
...
(keybindings-org-table-set magit-keys 'jw-magit-keys)
#+END_SRC
...
| Key       | Function                   | Scope |
|-----------+----------------------------+-------|
| C-c <     | org-time-stamp             | local |
| C-c .     | org-time-stamp-inactive    | local |
| C-c C-x g | org-feed-update-all-or-one | local |

#+HEADER: :var org-hook-keys=org-hook-keys
...
(keybindings-org-table-set org-keys 'jw-org-keys)

(keybindings-org-table-cache-binding org-hook-keys 'jw-org-hook-keys)
(add-hook 'org-mode-hook (lambda ()
                           (text-scale-set 2)
                           (keybindings-org-table-set jw-org-hook-keys)))
...
| Key   | Function           | Scope |
|-------+--------------------+-------|
| C-c s | sbt-command-prefix | local |

#+HEADER: :var sbt-keys=sbt-keys
#+HEADER: :var sbt-hook-keys=sbt-hook-keys
#+BEGIN_SRC emacs-lisp
(defun sbt ()
  (interactive)
...
(keybindings-org-table-set sbt-keys 'jw-sbt-keys)

(keybindings-org-table-cache-binding sbt-hook-keys 'jw-sbt-hook-keys)
(defun sbt-turn-on-extra-bindings ()
  (interactive)
  (keybindings-org-table-set jw-sbt-hook-keys))
...
(keybindings-org-table-set sonic-pi-keys 'jw-sonic-pi-keys)
