...
(defun font-size-set ()
  ...
  (let* ((current (jw--get-font-size))
         (updated (read-string (format "Size (8 to 72, default 14, big 18, current %s): " current) current)))
...
(defun transparency-set ()
  (interactive)
   (let* ((current (jw--get-transparency))
          (updated (read-string (format "Transparency (0 - 100, default 75, current %s): " current) current)))
     (jw--set-transparency updated)))

(defun transparency-normal ()
  (interactive)
  (jw--set-transparency "75"))

(defun transparency-off ()
  (interactive)
  (jw--set-transparency "100"))
