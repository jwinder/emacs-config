...
(defun jw--set-font-size (size)
  (when (and (numberp size) (>= size 8) (<= size 80))
    (setq jw--font-size size)
    (set-face-attribute 'default nil :font (jw--font-name size))))

(setq jw--transparency-alpha nil)

(defun jw--get-transparency-alpha ()
  jw--transparency-alpha)

(defun jw--set-transparency-alpha (value)
  (when (and (numberp value) (>= value 0) (<= value 100))
    (setq jw--transparency-alpha value)
...
(defun font-size-set ()
  ...
  (let* ((current (jw--get-font-size))
         (updated (read-string (format "Font size 8 to 80, default 14, current %s: " current) (number-to-string current))))
...
(defun transparency-alpha-set ()
  (interactive)
   (let* ((current (jw--get-transparency-alpha))
          (updated (read-string (format "Transparency alpha 0 to 100, default 75, current %s: " current) (number-to-string current))))
     (jw--set-transparency-alpha (string-to-number updated))))

(defun transparency-alpha-normal () (interactive) (jw--set-transparency-alpha 75))
...
(defun font-size-transparency-alpha-normal ()
  (interactive)
  (font-size-normal)
  (transparency-alpha-normal))
...
(define-key ctl-x-4-map (kbd "R") 'font-size-transparency-alpha-normal)
...
(global-set-key (kbd "<C-wheel-right>") 'transparency-alpha-increase)
(global-set-key (kbd "<C-wheel-left>") 'transparency-alpha-decrease)
...
(defun sbt:command--advice--message-command (original-function &rest args)
  ...
  (apply original-function args))
(advice-add 'sbt:command :around 'sbt:command--advice--message-command)
...
(yas-reload-all)
(scratch-buffer)
(font-size-transparency-alpha-normal)
(cd (getenv "HOME"))
