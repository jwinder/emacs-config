...
(transient-define-prefix jw-runbook-menu ()
  ["Runbook"
   ...
   ("$" "repl           open project's language repl" jw-runbook-run-repl)
   ]
  ["Shell"
...
(cl-defun jw-runbook-setup (&key runbook-type runbook-name install start run compile test-compile test test-only lint command last-command repl shell-command eshell cleanup open-jenkins)
  (declare (indent defun))
