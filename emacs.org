...
(use-package vertico) ;; replaces ivy
(use-package orderless) ;; search engine in ivy
(use-package marginalia) ;; annotations in ivy
(use-package consult) ;; replaces swiper, counsel search, etc

;; (use-package hydra) ;; todojoe -- this may have been required by treemacs, but i don't think i need it anymore since switching to dired-subtree

(use-package dired-subtree) ;; other possibly useful extensions: https://github.com/Fuco1/dired-hacks
...
(when (jw--is-wsl-linux)
  (defun jw--get-wsl-linux-bridge-ip ()
    "Get the actual WSL bridge IP (vEthernet) from the route table."
    (string-trim (shell-command-to-string "ip route show | grep default | awk '{print $3}'"))))
#+END_SRC
...
(setq-default font-lock-multiline t) ;; supposedly faster, enables caching of batches of lines instead of font-locking line by line.
...
;; old interactive functions to kill
(fmakunbound 'woman)
(fmakunbound 'woman-find-file)
(fmakunbound 'woman-dired-find-file)
#+END_SRC
...
;; (global-set-key (kbd "C-M-/") 'counsel-company) ;; todojoe
...
(setq dired-subtree-use-backgrounds nil) ;; do not highlight background
...
;; (define-key dired-mode-map (kbd "C-c i") 'dired-subtree-insert)
(define-key dired-mode-map (kbd "C-c i") 'dired-subtree-toggle)
(define-key dired-mode-map (kbd "C-c ^") 'dired-subtree-remove)
(define-key dired-mode-map (kbd "C-c m") 'dired-subtree-mark-subtree)
(define-key dired-mode-map (kbd "C-c u") 'dired-subtree-unmark-subtree)
#+END_SRC
...
(remove-hook 'git-commit-setup-hook 'git-commit-setup-auto-fill) ;; stop auto-wrapping of COMMIT_MSG extra body message after 68 chars
...
;; these are important for maintaining good performance in large org files
(setq org-adapt-indentation nil ;; causes scans to make sure text is aligned correctly. i don't use this anyway b/c i like to type fully left-justified after headings.
      org-fontify-emphasized-text nil ;; can cause high performance hit, only used for fontifying bold & italic.
      org-fontify-whole-heading-line nil ;; this can cause performance issues and doesn't do anything with my current color theme.
      org-refile-targets '((org-agenda-files :maxlevel . 5))
      )

;; use org-indent-mode via hook
...
(org-babel-src-yasnippet 'gdscript)
...
use org-mode tables in markdown-mode
#+BEGIN_SRC emacs-lisp
(add-hook 'markdown-mode-hook 'turn-on-orgtbl)

(defun jw-markdown-org-table-fix ()
  "Convert org-mode table intersections (+) to markdown table pipes (|)"
  (interactive)
...
(add-hook 'markdown-mode-hook
          (lambda ()
            (add-hook 'before-save-hook 'jw-markdown-org-table-fix nil t)))
#+END_SRC
...
(setq ;;vertico-count-format '("%d/%d " . "%d/%d ")
      vertico-resize t)
...
(set-face-attribute 'vertico-current nil  :background "#3e4451"  :foreground 'unspecified)
...
(global-set-key (kbd "M-M-y") 'consult-yank-pop)
(global-set-key (kbd "C-M-y") 'consult-yank-pop)
(global-set-key (kbd "C-c C-j") 'consult-org-heading)

;; ("C-x b" . consult-buffer)           ;; Replaces counsel-switch-buffer (handles virtuals natively)
;;          ("C-x p" . list-processes)           ;; You can write a DWIM for this later if needed

;;          ;; Search / Swiper replacements
;;          ("C-s"   . consult-line)             ;; Replaces swiper
;;          ("C-r"   . consult-line)             ;; (Can also just be consult-line, it searches whole buffer)
;;          ("M-s i" . consult-imenu)            ;; Replaces counsel-imenu
;;          ("M-s s" . consult-ripgrep)          ;; Replaces counsel-rg
;;          ("M-s g" . consult-grep)             ;; Replaces counsel-grep

;;          ;; Misc
;;          ("C-M-y"   . consult-yank-pop)       ;; Replaces counsel-yank-pop
...
scala3 hack to revert to basic indent
#+BEGIN_SRC emacs-lisp
(defun jw--scala-mode--simplify-indent-for-scala3 ()
  "Revert to basic previous non-blank line relative indent for scala3.
For scala projects, this checks for \"scalaVersion := 3\" patterns in build.sbt.
For non-scala projects (scripts), this only loads when there is a \"// scala3\" in the file/buffer."
  (interactive)
  (let ((root (and (fboundp 'projectile-project-root) (projectile-project-root))))
      (when (or (save-excursion (goto-char (point-min)) (re-search-forward "scalaVersion\\|scala3" 2000 t))
                (and root (file-exists-p (expand-file-name "build.sbt" root))
                     (with-temp-buffer
                       (insert-file-contents (expand-file-name "build.sbt" root) nil 0 2000)
                       (goto-char (point-min))
                       (re-search-forward "scalaVersion\\s-*:=\\s-*\"?3" nil t))))
        ;; The Hack: Lobotomize scala-mode's indentation entirely
        (setq-local indent-line-function 'indent-relative-first-indent-point)
        (setq-local electric-indent-chars nil)
        (local-set-key (kbd "RET") 'newline-and-open-line-previous))))

(add-hook 'scala-mode-hook 'jw--scala-mode--simplify-indent-for-scala3)
#+END_SRC
...
(defun jw-js-package-json-scripts-as-functions ()
  ...
  (letrec ((default-directory (or (projectile-project-root) (jw--pwd)))
    ...
    ;; (counsel-M-x (format "package-json--%s" dir-name)) ;; todojoe maybe execute-extended-command
    ))
...
(defun jw-project-insert-golang-commands-as-org-links ()
  (interactive)
  (let ((commands
         '(("go mod tidy" . "Remove unused modules and add missing ones")
           ("go mod download" . "Download modules to local cache")
           ("go mod graph" . "Print the module dependency graph")
           ("go fmt ./..." . "Format all code in the project")
           ("golangci-lint run -v ./..." "Run static analysis on project (check for bugs & styles)")
           ("go vet ./..." . "Report suspicious constructs (static analysis)")
           ("go build ./..." . "Compile packages to check for errors")
           ("go test -v ./..." . "Run all tests with verbose output")
           ("go test -race ./..." . "Run tests with the data race detector")
           ("go clean -cache -modcache" . "Clear build artifacts and downloaded modules")
           ("go clean -testcache" . "Clear cached test results")
           ("$ go test ./dir -count=1 -v" . "Test specific directory")
           ("$ go get github.com/org/repo@latest" . "Update single dependency"))))
    (dolist (pair commands)
      (let* ((raw-cmd (car pair))
             (desc (cdr pair))
             (is-text-only (string-prefix-p "$" raw-cmd)))
        (if is-text-only
            (insert (format "%s # %s\n" raw-cmd desc))
          (insert (format "- [[elisp:(compile \"%s\")][%s]] -- %s\n"
                          raw-cmd raw-cmd desc)))))))
...
(defun jw-project-insert-sbt-commands-as-org-links ()
  ...
  (dolist (command
           '(("sbt" . "jw-sbt") ("compile main" . "jw-sbt-compile") ("compile test" . "jw-sbt-compile 'test") ("test all" . "jw-sbt-test 'all")
             ("scalafmtAll" . "sbt-command \"scalafmtAll\"") ("scalafmt" . "sbt-command \"scalafmt\"") ("scalafmtSbt" . "sbt-command \"scalafmtSbt\"") ("scalafix" . "sbt-command \"scalafix\"")
             ("githubWorkflowGenerate" . "sbt-command \"githubWorkflowGenerate\"")
    ...
    (insert (format "- [[elisp:(%s)][%s · %s]]\n" (cdr command) (s-pad-right 24 " " (car command)) (cdr command)))))
...
(defun jw-project-add-org-links-inserter-link ()
  ...
  (let ((inserter-link-fmt-string "\n\n[[elisp:(progn (open-line-next) (newline) (%s))][%s]]"))
    ...
    (when (file-exists-p "go.mod")
      (insert (format inserter-link-fmt-string "jw-project-insert-golang-commands-as-org-links" "jw-project-insert-golang-commands-as-org-links")))
    (when (file-exists-p "Makefile")
  ...
  (letrec ((default-directory (or (projectile-project-root) (jw--pwd)))
    ...
    (dolist (targets-and-impls (jw-make--get-targets-and-impls-list))
      (let ((command-name (format "%s %s" script-runner (car targets-and-impls)))
        ...
        (funcall make-fn command-name command-impl)))
    ;; (counsel-M-x (format "makefile--%s" dir-name)) ;; todojoe maybe execute-extended-command
    ))
#+END_SRC
