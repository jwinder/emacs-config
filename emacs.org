...
(cl-defun jw--run-cmd (command &key process-name buffer-name before-process-creation after-process-creation after-process-finish tail-output delete-tmp-script)
...
"
  ...
  (let* ((prepared-cmd (string-trim command))
         ...
         (cmd-buffer-name (or buffer-name (format "*%s*" cmd-process-name))))
...
(defun jw-js-yarn-command (arg &optional command)
  ...
  (let* ((cmd-start (if (or arg (not (jw-js--binci-exists))) "yarn" "binci"))
    ...
    (message cmd)
    (jw--run-cmd cmd :buffer-name (format "*%s*<%s>" cmd (jw-js--get-project-name)) :tail-output t)))

(jw-runbook-setup-type
 "js"
 :install (lambda (arg) (jw-js-yarn-command arg "install"))
 :start (lambda (arg) (jw-js-yarn-command arg "dev")) ;; todojoe -- delegate to dev or start depending on the contents of package.json. elgot/lsp startup.
 :run 'ignore ;; todojoe
