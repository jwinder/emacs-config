...
fix for `helm-list-emacs-process` adding unwanted header to top of bufffer
`list-processes--refresh` calls `tabulated-list-init-header` which adds a header
originally this function was inside of `:init` and added a header to the unwanted buffer
#+BEGIN_SRC emacs-lisp
(setq helm-source-emacs-process
...
ob-scala copied from [[https://github.com/jwiegley/org-mode/blob/master/lisp/ob-scala.el][org-mode's github]] since scala-mode's ob-scala requires ensime
#+BEGIN_SRC emacs-lisp
