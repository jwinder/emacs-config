...
(cl-defun jw--run-cmd (command &key process-name before-process-creation after-process-creation after-process-finish delete-tmp-script beginning-of-buffer)
          ...
          (let ((process-buffer (get-buffer-create cmd-buffer-name)))
            ...
            (let ((proc (get-buffer-process process-buffer)))
              ...
              (jw--do-when-process-finishes (get-buffer-process process-buffer)
                                            `(lambda (proc)
                                               (when ,beginning-of-buffer (beginning-of-buffer))
...
(defun cheat-sh (&optional arg)
  ...
  (let* ((raw-query
         ...
         (query (url-encode-url (s-replace "\s" "+" (s-trim raw-query)))))
    (jw--run-cmd (format "curl http://cheat.sh/%s" query) :beginning-of-buffer t)))
