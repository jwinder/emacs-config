...
#+BEGIN_SRC emacs-lisp
(setq log-this-command--last-log nil)

(defun log-this-command (&rest _)
  (let* ((keys (key-description (this-command-keys)))
         (cmd (jw--symbol-name this-command))
         (log (concat (s-pad-right 24 "\s" keys) cmd))
         (buffer (get-buffer-create "*command-log*")))
    (unless (equal log-this-command--last-log log)
      (setq log-this-command--last-log log)
