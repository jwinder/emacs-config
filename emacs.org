...
(defun jw--make-temporary-script (command)
  (let* ((temporary-script-relative-filename (replace-regexp-in-string "[^a-zA-Z0-9]+" "-" command))
         (jw-temporary-script-dir (concat temporary-file-directory "jw-tmp-scripts/"))
         (temporary-script-absolute-filename (concat jw-temporary-script-dir temporary-script-relative-filename))
         (script-contents (format "#!/bin/sh\n\ncd %s\n\n%s" (jw--pwd) command)))
    (unless (file-exists-p jw-temporary-script-dir) (make-directory jw-temporary-script-dir))
    (jw--write-to-file script-contents temporary-script-absolute-filename)
    (unless (file-executable-p temporary-script-absolute-filename) (shell-command (format "chmod u+x %s" temporary-script-absolute-filename)))
    temporary-script-absolute-filename))

(defun jw--run-cmd (command &optional buffer-name before-process-creation after-process-creation)
...
For long running processes, this will always toggle back and forth between the process buffer and the other buffer as long as the process is alive."
  (let* ((prepared-cmd (string-trim command))
         (command-tokens (split-string prepared-cmd))
         (command-name (car command-tokens))
         (command-args (cdr command-tokens))
         (name-for-buffer (or buffer-name (concat "*" (replace-regexp-in-string "[\t\n\r]+" " " prepared-cmd) "*")))
         (command-script (jw--make-temporary-script prepared-cmd))
         (process-is-alive (process-live-p (get-process command-script))))
    (if (string= "" prepared-cmd)
        (message "Empty command! Did nothing.")
      (if (and process-is-alive (string= (buffer-name) name-for-buffer))
          (switch-to-buffer (other-buffer))
        (progn
          (when (and (not process-is-alive) (get-buffer name-for-buffer)) (kill-buffer name-for-buffer))
          (if (get-buffer name-for-buffer)
              (switch-to-buffer name-for-buffer)
            (let ((buffer (get-buffer-create name-for-buffer)))
              (when before-process-creation (funcall before-process-creation))
              (switch-to-buffer buffer)
              (apply 'make-comint-in-buffer command-script buffer command-script nil command-args)
              (when after-process-creation (funcall after-process-creation))))))
      (message "Running command %s" prepared-cmd))))

;; todo -- remote tmux sessions?
...
(defvar sonic-pi-server-buffer-name "*sonic pi*")
...
(defun sonic-pi-server-cleanup-process-buffer ()
  (interactive)
  (when (and (not (sonic-pi-server-running-p)) (get-buffer sonic-pi-server-buffer-name))
    (kill-buffer sonic-pi-server-buffer-name)))

(defun sonic-pi-server ()
  (interactive)
  (sonic-pi-server-cleanup-process-buffer)
  (jw--run-cmd "sonic-pi run-server" sonic-pi-server-buffer-name nil
               '(lambda ()
