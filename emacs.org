...
(defun mode-line-toggle-pwd ()
  (interactive)
  (setq jw-mode-line-config-show-pwd (not jw-mode-line-config-show-pwd)))
...
(defun toggle-mode-line-visibility (&optional local)
  (interactive "P")
  (if mode-line-format
      (mode-line-off local)
    (mode-line-on local)))
...
(define-key ctl-x-5-map (kbd "C-<return>") 'toggle-mode-line-visibility)
...
(define-prefix-command 'jw-js-keymap)
(global-set-key (kbd "C-c n") 'jw-js-keymap) ;; n = node
...
(defun counsel-find-file-dwim (arg)
  (interactive "P")
  (if arg (counsel-recentf)
    (if (region-active-p) (counsel-find-file (buffer-substring-no-properties (region-beginning) (region-end)))
      (counsel-find-file))))
