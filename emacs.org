...
(jw-runbook-setup-type
 ...
 :cleanup (lambda (arg)
            ...
            (jw--run-cmd "docker compose stop && docker compose -f rm" :after-process-finish (lambda (proc) (jw--kill-process-buffer proc))))
            )
#+END_SRC
...
(defun jw-js-run-raw-command (arg &optional command)
  (interactive "P")
  (message command)
  (setq jw-js--last-command-global command)
...
(defun jw-js--find-entrypoint ()
  (-find 'f-exists? (list "index.js" "server.js" "app.js" "src/index.js" "src/server.js" "src/app.js")))
