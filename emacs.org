...
`:before-process-creation' is an optional zero-arg lambda that is run before the process is started.
`:after-process-creation' is an optional one-arg lambda (providing the process an arg) that is run after the process is started.
`:after-process-finish' is an optional one-arg lambda (providing the process as an arg) that is run after the process finishes.
...
Warning: The optional lambda functions currently MUST be lambdas, not symbols with function definitions.
"
  (require 'comint)
...
(defun jw-todo-backup (arg)
  ...
  (jw--run-cmd "todo-backup"
               :after-process-finish `(lambda (proc)
                                        (when ,(not arg) (jw--kill-process-buffer proc))
...
(defun jw-runbook-run-dev (arg) (interactive "P") (jw-runbook--find-and-run-function "dev" arg))
...
(define-key jw-runbook-keymap (kbd "d") 'jw-runbook-run-dev)
...
(transient-define-prefix jw-runbook-menu ()
  ["Runbook"
   ...
   ("d" "dev             start development dependencies, e.g. lsp" jw-runbook-run-dev)
   ("s" "start           start development server locally" jw-runbook-run-start)
...
(cl-defun jw-runbook-setup-type (type &key install dev start run compile test-compile test test-only lint command last-command repl shell-command eshell cleanup)
  (interactive)
  (when install (fset (intern (format "jw-runbook-run-install:%s" type)) install))
  (when dev (fset (intern (format "jw-runbook-run-dev:%s" type)) dev))
...
#+BEGIN_SRC emacs-lisp
(defun jw-js--binci-exists () (f-exists? "binci.yml"))

(defun jw-js-yarn-command (arg &optional command)
  (interactive "P")
  (let* ((cmd-start (if (or arg (not (jw-js--binci-exists))) "yarn" "binci"))
         (cmd (if command (concat cmd-start " " command) (read-shell-command "Run: " (concat cmd-start " ")))))
    (message cmd)
    (jw--run-cmd cmd :tail-output t)))

(jw-runbook-setup-type
 "js"
 :install (lambda (arg) (jw-js-yarn-command arg "install"))
 :dev 'ignore ;; todojoe -- elgot/lsp stuff
 :start (lambda (arg) (jw-js-yarn-command arg "dev")) ;; todojoe -- delegate to dev or start depending on the contents of package.json? maybe just roll lsp stuff into start and kill :dev command.
 :run 'ignore ;; todojoe
 :compile 'ignore ;; todojoe
 :test (lambda (arg) (jw-js-yarn-command arg "test")) ;; todojoe -- compilation mode
 :test-only (lambda (arg) (jw-js-yarn-command arg "test"))
 :lint (lambda (arg) (jw-js-yarn-command arg "lint"))
 :command (lambda (arg) (jw-js-yarn-command arg))
 :last-command 'ignore
 :repl (lambda (arg) (jw--run-cmd "node" :tail-output t))
 :cleanup (lambda (arg)
            (projectile-kill-buffers)
            (when (jw-js--binci-exists)
              (jw--run-cmd "binci --cleanup"
                           :after-process-finish (lambda (proc) (jw--kill-process-buffer proc))))
            ))
#+END_SRC
