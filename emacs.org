...
(pkg 'counsel) ;; brings in ivy & swiper
(pkg 'counsel-projectile)
...
;; todo emacs 28.1 -- dumb-jump xref backend and use M-. instead of M-s j -- I can install an updated xref from elpa, but I'm too lazy to switch this now.
;; :config (lambda ()
