...
(use-package xah-math-input
  :config (progn
            (xah-math-input--add-to-hash [ ["def" "≝"] ["pm" "±"] ["cdot" "·"] ["wedge" "∧"]])
...
(defun jw--do-when-process-finishes (process fn)
  "Invoke function `fn' after process `process' finishes or exits, having the current buffer focus on the process buffer for `process'."
  (when process
      ...
      (if (process-live-p (get-process cmd-process-name))
        ...
        (let* ((tmp-script-dir "/tmp/emacs-jw-run-cmd/")
          ...
          (let ((process-buffer (get-buffer-create cmd-buffer-name)))
            ...
            (when after-process-finish (jw--do-when-process-finishes (get-process cmd-process-name) `(lambda () (funcall ,after-process-finish))))
...
(defun eshell--prompt-function ()
  (let ((dir-string (propertize (abbreviate-file-name (eshell/pwd)) 'face '(:foreground "CornflowerBlue")))
    ...
    (s-collapse-whitespace (format "%s %s %s %s" dir-string git-string prompt-string right-pad-string))))
...
(defun ansi-term--exit-hook ()
  (jw--do-when-process-finishes (get-buffer-process (current-buffer))
                                (lambda ()
...
(org-babel-support-langs
 (list 'awk 'calc 'C 'dot '(emacs-lisp elisp el) '(haskell hask hs) 'http 'java 'js 'latex 'lisp
