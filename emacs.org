...
(defun jw-js--make-process-name (process-name) (format "%s-%s" (jw-js--get-project-name) process-name))
(defun jw-js--make-buffer-name (process-name) (format "*%s*<%s>" process-name (jw-js--get-project-name)))
...
(setq jw-js--last-raw-command nil) ;; todo -- doesn't cache last command per project.
(defun jw-js-run-raw-command (arg &optional command)
  (interactive "P")
  (message command)
  (setq jw-js--last-raw-command command)
  (let ((cmd (or command (read-shell-command "Run: "))))
    (jw--run-cmd cmd
                 :process-name (jw-js--make-process-name cmd)
                 :buffer-name (jw-js--make-buffer-name cmd)
                 :tail-output t)))
...
(jw-runbook-setup-type
 ...
 :start (lambda (arg) (jw-js--run-first-command arg (list "start" "dev"))) ;; todojoe -- elgot/lsp.
 :run nil ;; todojoe "node index.js/server.js" or something
 ...
 :last-command (lambda (arg) (jw-js-run-raw-command arg jw-js--last-raw-command))
