...
(jw-runbook-setup-type
 ...
 :cleanup (lambda (arg)
            ...
            (jw--run-cmd "docker compose stop && docker compose -f rm" :after-process-finish 'jw--kill-process-buffer)))
#+END_SRC
...
(jw-runbook-setup-type
 ...
 :cleanup (lambda (arg)
            (projectile-kill-buffers)
            (when (jw-js--binci-p) (jw--run-cmd "binci --cleanup" :after-process-finish 'jw--kill-process-buffer))))
#+END_SRC
