...
Now, try starting emacs. Note that this repository does not contain byte-compiled files. If you would like to byte-compile each elisp file, inside of emacs run the function I have prepared: `M-x emacs-byte-compile-elpa-package-files` (might take a minute to run).
