...
;; helm-occur to follow the source lines much like helm-swoop
(defvar helm-occur-follow-sources-dwim-record ())
(add-to-list 'helm-occur-follow-sources-dwim-record 'helm-source-occur)

(defun helm-occur-follow-sources-dwim ()
  (mapc (lambda (source)
          (when (memq source helm-occur-follow-sources-dwim-record)
            (helm-attrset 'follow 1 (symbol-value source))))
        helm-sources))
(add-hook 'helm-before-initialize-hook 'helm-occur-follow-sources-dwim)
