...
C-u 4 M-x `cmd-dwim' will run a shell command and insert the response in the buffer on the next line.
...
Interactively:
 ...
 - Otherwise, the shell command is read from prompt."
  ...
  (let ((prepared-cmd (or command (if (region-active-p)
    ...
    (if arg
        (case arg
          (4 (save-excursion (open-line-next) (insert (s-trim (shell-command-to-string prepared-cmd)))))
