...
;; the default action that switches to either a buffer or file is slow as hell, even in small repos.
;; changing it to find only files. NOTE: this does not change the counsel menu description.
;; counsel-projectile itself is still slow, but i don't use it much.
(fset 'counsel-projectile-switch-project-action 'counsel-projectile-switch-project-action-find-file)
