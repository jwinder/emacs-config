...
Custom settings can be placed inside of a =private= directory of elisp files, or a =private.org= file of elisp code blocks.
#+BEGIN_SRC sh :tangle no
...
Once emacs is loaded, configurations can be byte-recompiled using the following helper functions:
#+BEGIN_SRC emacs-lisp :tangle no
(emacs-byte-compile-elpa-package-files)
(emacs-byte-compile-init-files)
#+END_SRC

To install the latest packages, remove the emacs =elpa= directory and restart.
...
(defun emacs-byte-compile-init-files ()
  (interactive)
  (byte-recompile-file user-init-file 'force 0 'reload)
  (byte-recompile-file jw--init-file 'force 0 'reload)
  (when (file-exists-p jw--init-private-file)
    (byte-recompile-file jw--init-private-file 'force 0 'reload)))
#+END_SRC
