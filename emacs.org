...
(cl-defun jw--run-cmd (command &key process-name before-process-creation after-process-creation after-process-finish delete-tmp-script-after)
...
;; crypt prompts to match: "Enter encryption key:" and "Enter encryption key: (repeat)"
(setq comint-password-prompt-regexp (concat comint-password-prompt-regexp "\\|^Enter encryption key:.*\\s *\\'"))
...
(defun weather (&optional arg)
  ...
  (let* ((raw-query
          (pcase (prefix-numeric-value arg)
            ...
            (_ "")))
         (query (s-replace "\s" "+" (s-trim raw-query))))
