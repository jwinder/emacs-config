...
(use-package transient
  :config (setq transient-enable-popup-navigation t
  ...
  (let* ((root-dir (or jw-runbook--target-location (projectile-project-root)))
    ...
    (if (not type) (message "Could not find project type for jw-runbook")
      (let* ((func-name (format "jw-runbook-run-%s:%s" name type))
        ...
        (cond
         ((fboundp func) (let ((default-directory root-dir)) (funcall func a)))
         ((fboundp default-func) (let ((default-directory root-dir)) (funcall default-func a)))
         (t (message "jw-runbook - neither %s nor %s functions bound" func-name default-func-name)))))))
...
(define-key jw-runbook-keymap (kbd "j") 'jw-runbook-run-open-jenkins)
...
(transient-define-prefix jw-runbook-menu ()
  ...
  ["Runbook deployment"
   ("j" "open-jenkins    open jenkins job in web browser" jw-runbook-run-open-jenkins)
   ])
...
(cl-defun jw-runbook-setup-type (type &key install start run compile test-compile test test-only lint command last-command repl shell-command eshell cleanup open-jenkins)
  ...
  (when shell-command (fset (intern (format "jw-runbook-run-shell-command:%s" type)) shell-command))
  (when eshell (fset (intern (format "jw-runbook-run-eshell:%s" type)) eshell))
  (when cleanup (fset (intern (format "jw-runbook-run-cleanup:%s" type)) cleanup))
  (when open-jenkins (fset (intern (format "jw-runbook-run-open-jenkins:%s" type)) open-jenkins)))
...
(jw-runbook-setup-type
 ...
 :cleanup (lambda (arg)
            ...
            (jw--run-cmd "docker compose stop" :kill-process-buffer t)))
#+END_SRC
...
(jw-runbook-setup-type
 ...
 :cleanup (lambda (arg)
            (projectile-kill-buffers)
            ;; (when (jw-js--binci-p) (jw--run-cmd "binci --cleanup" :kill-process-buffer t)) ;; todo -- this does't quite work for some reason
            ))
#+END_SRC
