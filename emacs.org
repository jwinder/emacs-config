...
If `run-command-always' is non-nil then we always run the command even if the target process buffer already exists."
  (if (and just-toggle-back-if-current-buffer-matches buffer-name (string= (buffer-name) buffer-name))
      (switch-to-buffer (other-buffer))
    (if (and buffer-name (get-buffer buffer-name) (not run-command-always))
...
(defun cmd (command)
  ...
  (jw--run-cmd command nil nil nil nil t))
