...
(use-package inf-ruby
  :config (setq inf-ruby-default-implementation (if (executable-find "pry") "pry" "ruby")))
...
Example =env-path= file, symlinked to =~/.emacs.d/env-path= from a safe place
#+BEGIN_EXAMPLE
$HOME/bin
/usr/local/bin
/usr/bin
...
(defun comment-dwim-dwim (&optional arg)
  "When the region is active, then toggle comments over it. Otherwise, toggle commenting the current line. If there is a prefix arg, then append a comment to the end of the line instead. If the prefix arg is 4, then kill the comment on the current line."
  (interactive "*P")
  (if (region-active-p)
      (comment-dwim arg)
    (if arg
...
(defun rcirc-connect-dwim (&optional server port nick user-name full-name startup-channels password encryption)
  "Alternative to `rcirc-connect' which toggles to the server if already connected. If no server is provided, then a prompt will ask the user for a server."
  (interactive)
  (if server
...
=rcirc-server-alist= is used when calling =rcirc= without prefix arg
